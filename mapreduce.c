...
void* threads_reducer(void* arg){
    ...
    hash_map[partition_number].sorted_keys = malloc(sizeof(k) * hash_map[partition_number].num_keys);
    for (int i = 0,ptr = 0; i < 1024; i++){
    ...
    for (int i = 0; i < hash_map[partition_number].num_keys; i++){
...
void MR_Emit(char *key, char *value){
    ...
    v* vnode = malloc(sizeof(v));
    if (vnode == NULL) {
        perror("malloc failed @ Emit");
        pthread_mutex_unlock(&hash_map[partition_number].list[map_number].lock);
        return;
    } 
    vnode->value = malloc(sizeof(char)*20);
    strcpy(vnode->value, value);
    vnode->next = NULL;
    if (temp == NULL){    //no existing node for same key
        k *knode = malloc(sizeof(k));
        if (knode == NULL) {
            perror("malloc failed @ Emit");
            pthread_mutex_unlock(&hash_map[partition_number].list[map_number].lock);
            return;
        }
        knode->head = vnode, knode->next = hash_map[partition_number].list[map_number].head, knode->key = malloc(sizeof(char)*20), hash_map[partition_number].list[map_number].head = knode;
